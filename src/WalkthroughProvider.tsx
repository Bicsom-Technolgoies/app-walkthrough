...
export const WalkthroughProvider = ({
  ...
  const registryRef = useRef<Map<number, Omit<WalkthroughStep, "layout">>>(
    new Map(),
  );
  // Layouts: zone number → measured layout
  const layoutsRef = useRef<Map<number, StepLayout>>(new Map());

  const [isRunning, setIsRunning] = useState(false);
  const [sortedSteps, setSortedSteps] = useState<WalkthroughStep[]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);

  // Derived: merge registry + layouts into sorted steps
  const buildSteps = useCallback((): WalkthroughStep[] => {
    const steps: WalkthroughStep[] = [];
    registryRef.current.forEach((spec, zone) => {
      const layout = layoutsRef.current.get(zone);
      if (layout) steps.push({ ...spec, layout });
    });
  ...
  const registerStep = useCallback(
    (spec: Omit<WalkthroughStep, "layout">, layout: StepLayout) => {
    ...
    (fromZone = 1) => {
      ...
      requestAnimationFrame(() => {
        ...
        const startIdx = Math.max(
          0,
          steps.findIndex((s) => s.zone >= fromZone),
        );
