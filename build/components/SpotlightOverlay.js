...
import { Modal, View, Dimensions, StyleSheet, StatusBar, Text, } from "react-native";
...
const getPaddingValue = (padding) => {
    ...
    return {
        ...
        bottom: padding.paddingBottom ?? padding.paddingVertical ?? padding.padding ?? 0,
        left: padding.paddingLeft ?? padding.paddingHorizontal ?? padding.padding ?? 0,
        right: padding.paddingRight ?? padding.paddingHorizontal ?? padding.padding ?? 0,
    };
};
const AnimatedRect = Animated.createAnimatedComponent(SvgRect);
const AnimatedCircle = Animated.createAnimatedComponent(SvgCircle);
export const SpotlightOverlay = ({ step, currentIndex, totalSteps, overlayColor, overlayOpacity, defaultBorderRadius, tooltipComponent: CustomTooltip, onNext, onPrev, onStop, arrowColor, }) => {
    ...
    const tooltipLeft = Math.max(20, Math.min(centerX - tooltipWidth / 2, W - tooltipWidth - 10));
    /* ---------- Shared Values ---------- */
    ...
    return (<Modal visible transparent statusBarTranslucent animationType="none">
      <View style={StyleSheet.absoluteFill}>
        <Animated.View style={[StyleSheet.absoluteFill, { opacity: fade.value }]} pointerEvents="none">
          <Svg width="100%" height="100%">
            <Defs>
              ...
              <RadialGradient id="glow" cx="50%" cy="50%" r="40%">
                ...
                <Stop offset="49.52%" stopColor="rgba(255,194,255,0.45)" stopOpacity={0.2}/>
                <Stop offset="100%" stopColor="rgba(255,194,255,0)" stopOpacity={0}/>
              </RadialGradient>
            </Defs>

            <SvgRect width="100%" height="100%" fill={overlayColor} opacity={overlayOpacity} mask="url(#mask)"/>

            {step.isGlow && (<AnimatedCircle fill="url(#glow)" animatedProps={animatedCircleProps}/>)}
          </Svg>
    ...
    >
            <Tooltip step={step} currentIndex={currentIndex} totalSteps={totalSteps} isFirstStep={currentIndex === 0} isLastStep={currentIndex === totalSteps - 1} onNext={onNext} onPrev={onPrev} onStop={onStop}/>
          </Animated.View>
